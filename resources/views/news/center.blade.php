...
@foreach($posts as $posts)
    <div class="uk-width-1-1 uk-margin-large-bottom news-center">
        <div class="uk-grid uk-grid-small">
            <div class="uk-width-1-5"><a href=""><img src="/upload/{{$posts->thumbs}}"></a></div>
            <div class="uk-width-7-10 uk-margin-left">
                <div class="ntitle"><a href="{{route('companyShow',$posts->id)}}">{{$posts->title}}</a></div>
                <div class="infos" style="height:48px; overflow:hidden">{{str_limit(strip_tags($posts->content),150)}}</div>
                <div class="info-bar"><div class="uk-display-inline-block"><i class="uk-icon-clock-o"></i> {{date_format($posts->updated_at,'Y-m-d')}}　<i class="uk-icon-eye"></i> {{$posts->clicks}}</div>
                    <div class="uk-align-right uk-display-inline-block"><a href="{{route('companyShow',$posts->id)}}">点击全文</a>　</div></div>
            </div>
        </div>
    </div>
    @endforeach
